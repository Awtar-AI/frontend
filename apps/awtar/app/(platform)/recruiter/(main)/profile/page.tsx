...
export default function RecruiterProfilePage() {
    ...
    const removeSkill = (skill: string) => {
        setProfile({
            ...profile,
            specializations: profile.specializations.filter((s) => s !== skill),
        });
    };
    ...
    const addSkill = () => {
        if (newSkill.trim() && !profile.specializations.includes(newSkill.trim())) {
            setProfile({
                ...profile,
                specializations: [...profile.specializations, newSkill.trim()],
            });
            setNewSkill("");
        }
    };
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setInfoDraft({ ...infoDraft, website: e.target.value })
                                        }
                                        className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                    />
                                </div>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                            {profile.specializations.map((skill) => (
                                <span
                                    key={skill}
                                    className="group flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-700 text-xs font-semibold rounded-full border border-blue-100"
                                >
                                    {skill}
                                    <button
                                        onClick={() => removeSkill(skill)}
                                        className="text-blue-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 text-sm leading-none"
                                    >
                                        ×
                                    </button>
                                </span>
