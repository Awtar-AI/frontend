...
export const createJobFormSchema = z
  .object({
    ...
    employmentType: z.enum(
      employmentTypeOptions.map((o) => o.value) as [string, ...string[]],
    ),
    experienceLevel: z.enum(
      experienceLevelOptions.map((o) => o.value) as [string, ...string[]],
    ),
    salaryType: z.enum(
      salaryTypeOptions.map((o) => o.value) as [string, ...string[]],
    ),
  ...
  .superRefine((data, ctx) => {
    if (!data.isRemote && (data.latitude == null || data.longitude == null)) {
      ctx.addIssue({
        code: "custom",
        path: ["latitude"],
...
export function toCreateJobPayload(data: CreateJobFormData): CreateJobPayload {
  ...
  return {
    ...
    latitude: data.isRemote ? undefined : (data.latitude ?? undefined),
    longitude: data.isRemote ? undefined : (data.longitude ?? undefined),
    is_remote: data.isRemote,
    employment_type: data.employmentType as CreateJobPayload["employment_type"],
    experience_level:
      data.experienceLevel as CreateJobPayload["experience_level"],
    ...
    min_salary:
      data.salaryType === "undisclosed" || data.minSalary == null
        ...
        : data.minSalary,
    max_salary:
      data.salaryType !== "range" || data.maxSalary == null
        ? undefined
        : data.maxSalary,
