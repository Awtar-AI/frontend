...
import { JobLocationDisplay } from "../../post-job/_components/JobLocationDisplay";
import { JobLocationPicker } from "../../post-job/_components/JobLocationPicker";
import {
...
function toFormDefaults(job: JobPostResponse): CreateJobFormData {
  const experienceLevel = (job.expirence_level ??
    job.experience_level ??
    "mid") as CreateJobFormData["experienceLevel"];
  const employmentType =
    job.employment_type as CreateJobFormData["employmentType"];
...
export default function RecruiterJobDetailPage({
  ...
  const applicationsQuery = useRecruiterJobApplications(
    jobId,
  ...
  if (!organizationId) {
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
        Your session does not currently have an active organization. Please sign
        in again to manage this job.
      </div>
    );
  }
      ...
      <div className="space-y-6">
        ...
        {!jobQuery.data.is_remote && (
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            <JobLocationDisplay
              latitude={jobQuery.data.latitude}
              longitude={jobQuery.data.longitude}
                    ...
                    <div className="xl:col-span-12 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                        ...
                        {applicationsQuery.isError && (
                            <p className="text-sm text-red-600">
                                Could not load applicants for this job. If you saw 404 in network,
                                this specific job id may not exist on the backend you are connected
                                to.
                            </p>
                        )}
                        {!applicationsQuery.isLoading &&
