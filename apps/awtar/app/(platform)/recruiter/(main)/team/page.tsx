...
const INITIAL_INVITATIONS: Invitation[] = [
    { id: 1, email: "michael.ross@design.co", role: "Recruiter", sentDate: "Oct 12, 2023", status: "Pending" },
    { id: 2, email: "lisa.vogel@tech.io", role: "Admin", sentDate: "Oct 08, 2023", status: "Pending" },
    { id: 3, email: "david.wu@global.net", role: "Reviewer", sentDate: "Sep 24, 2023", status: "Expired" },
];
...
export default function TeamMembersPage() {
    ...
    const resendInvite = (id: number) => {
        setInvitations(invitations.map(inv =>
            inv.id === id
                ? { ...inv, status: "Pending", sentDate: new Date().toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" }) }
                : inv
        ));
    };
        ...
        <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="flex items-start justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Manage and invite recruiters to your talent acquisition team.</p>
                </div>
                <button
            ...
            {/* Invite Form */}
            <div id="invite-form" className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <h2 className="text-base font-bold text-gray-900 mb-1">Invite New Recruiter</h2>
                <p className="text-xs text-gray-500 mb-5">New members will receive an email invitation to join your workspace.</p>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-gray-500">{invitations.length} Total</span>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50 border-b border-gray-100">
                            <tr>
                                {["Email Address", "Role", "Sent Date", "Status", "Action"].map(h => (
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {invitations.map(inv => (
                                <tr key={inv.id} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-4 text-sm font-semibold text-gray-800">{inv.email}</td>
                                    <td className="px-6 py-4 text-sm text-gray-600">{inv.role}</td>
                                    <td className="px-6 py-4 text-sm text-gray-600">{inv.sentDate}</td>
