...
import { organizationsPublicApi } from "../../(jobs)/public-jobs/api/organizations-public.api";
import { publicJobsApi } from "../../(jobs)/public-jobs/api/public-jobs.api";
import { APPLICANT_ORG_PUBLIC_QUERY_KEY } from "../../(jobs)/public-jobs/hooks/use-organization-public";
import { APPLICANT_PUBLIC_JOB_QUERY_KEY } from "../../(jobs)/public-jobs/hooks/use-public-job";
import { formatExperienceLevelLabel } from "../../(jobs)/public-jobs/lib/format-job";
import type { PublicJob } from "../../(jobs)/public-jobs/schemas/public-jobs.schema";
...
function applicationTitle(app: ApplicationResponse): string {
	return app.current_job_title?.trim() || "Submitted application";
}
...
export default function ApplicationsPage() {
	...
	const jobById = useMemo(() => {
		const m = new Map<string, PublicJob>();
		applications.forEach((app, i) => {
			const job = jobMetaQueries[i]?.data;
			if (job) m.set(app.job_id, job);
		});
		return m;
	}, [applications, jobMetaQueries]);

	const orgMetaQueries = useQueries({
		queries: applications.map((_app, i) => {
			const organizationId = jobMetaQueries[i]?.data?.organization_id;
			return {
				queryKey: [...APPLICANT_ORG_PUBLIC_QUERY_KEY, organizationId] as const,
				queryFn: () => organizationsPublicApi.getPublic(organizationId ?? ""),
				enabled: Boolean(organizationId) && appsQuery.isSuccess,
				staleTime: 300_000,
			};
		}),
	});

	const orgNameByJobId = useMemo(() => {
		const m = new Map<string, string>();
		applications.forEach((app, i) => {
			const orgName = orgMetaQueries[i]?.data?.organization_name?.trim();
			if (orgName) m.set(app.job_id, orgName);
		});
		return m;
	}, [applications, orgMetaQueries]);
	...
	const filteredApplications = useMemo(() => {
		...
		return list;
	}, [applications, activeTab, search, jobTitleById, orgNameByJobId]);
			...
			{appsQuery.isLoading ? (
				...
				<div className="space-y-4">
					{filteredApplications.map((app) => {
						...
						const organizationName =
							orgNameByJobId.get(app.job_id) ?? "Hiring organization";

						return (
							<article
								key={app.id}
								className="w-full rounded-3xl border border-gray-200 bg-white p-6 text-left shadow-sm transition-colors hover:border-blue-400"
							>
								<div className="mb-4 inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1 text-[10px] font-bold text-blue-600">
								...
								<div className="mb-3 flex items-start justify-between gap-4">
									...
									<Link
										...
										className="inline-flex shrink-0 items-center gap-1.5 rounded-xl bg-gray-900 px-5 py-2 text-sm font-bold text-white shadow-md transition-colors hover:bg-black"
									>
								...
								<div className="mb-5 flex flex-wrap items-center gap-2.5 text-xs font-bold text-gray-500">
									{app.employment_type && (
										<>
											<span>{app.employment_type.replace(/_/g, " ")}</span>
									...
									<span>
										{job ? formatExperienceLevelLabel(job) : "Experience"} level
									</span>
								</div>

								<p className="mb-6 line-clamp-1 text-sm font-medium text-gray-600">
									{app.cover_letter
								...
								<div className="flex items-center justify-between gap-4">
									<span
										className={`inline-flex items-center gap-1.5 rounded-xl px-4 py-1.5 text-xs font-bold ${statusBadgeClass(app.status)}`}
									>
										<span
