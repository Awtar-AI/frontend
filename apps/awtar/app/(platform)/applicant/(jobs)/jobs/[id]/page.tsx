...
export default function JobDetailPage({
	...
	const isSaved = (savedJobsQuery.data?.data ?? []).some(
		(j) => j.job_id === jobId,
	);
	...
	const deadlinePassed = useMemo(
		() =>
			...
			new Date(job.deadline).getTime() < now,
		[job, now],
	);
	...
	const appliedAtLabel = existingApplication?.created_at
		? (() => {
				try {
					return new Intl.DateTimeFormat(undefined, {
						dateStyle: "medium",
					}).format(new Date(existingApplication.created_at));
				} catch {
					return existingApplication.created_at;
				}
			})()
		: null;
			...
			<div className="rounded-xl border border-gray-200 bg-white shadow-sm">
				<div className="p-8 lg:p-10">
					...
					<div className="flex flex-wrap items-center gap-2 mb-7">
						...
						{hasApplied && (
							<span className="inline-flex items-center gap-1.5 rounded-full border border-blue-100 bg-blue-50 px-3 py-1 text-[10px] font-black text-blue-700">
					...
					<div className="flex flex-col md:flex-row items-start gap-6 md:gap-8">
						...
						<div className="hidden md:flex flex-col items-end gap-3 shrink-0">
							<Link
								...
								className={`px-8 py-3.5 rounded-lg font-black text-sm transition-all whitespace-nowrap ${applyClass}`}
							>
								{applyLabel}
							</Link>
						</div>
					...
					{/* CTA — mobile */}
					{hasApplied && (
						<div className="mt-6 w-full rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-left text-xs font-bold leading-5 text-blue-800">
							You have already applied
							{appliedAtLabel ? ` on ${appliedAtLabel}` : ""}. Status:{" "}
					...
					<div className="md:hidden mt-6">
						<Link
							...
							className={`block w-full py-3.5 rounded-lg font-black text-sm text-center transition-all ${applyClass}`}
						>
							{applyLabel}
						</Link>
					</div>
							...
							{orgQuery.isLoading ? (
								...
								<>
									...
									<Link
										...
										className="flex w-full items-center justify-center gap-2 rounded-lg border border-slate-200 bg-slate-50 py-2.5 text-xs font-black text-slate-700 transition-all hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"
									>
					...
					<div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
						...
						<div className="p-6 space-y-3">
									...
									<div className="flex items-start gap-3">
										...
										<div>
											...
											<p className="mt-1 text-xs font-semibold leading-5 text-slate-500">
												{appliedAtLabel ? `Applied on ${appliedAtLabel}. ` : ""}
												Current status:{" "}
							...
							<Link
								...
								className={`block w-full rounded-lg py-3.5 text-center text-sm font-black transition-all ${applyClass}`}
							>
								{applyLabel}
							</Link>
						</div>
