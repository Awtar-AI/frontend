...
export async function request<T>( path: string, { method = "GET", params, headers = {}, body, ...rest}: RequestConfig){
      const url = buildUrl(path, params);

      const res = await fetch(url, {method, ...rest, 
        headers:{
...
export class ApiError extends Error{
    constructor(public status: number, public payload: unknown, public raw: string){
        super(typeof payload === "object" &&
            payload !== null && 
